...
namespace PIDL {
	...
	bool CPPWriter::write(Reader * reader, ErrorCollector & ec)
	{
		...
		case Mode::Include:
			{
				...
				CPPCodeGenContext ctx((*priv->o), CPPCodeGenContext::Mode::Declaration, (CPPCodeGenContext::Role)priv->role);
		...
		case Mode::Source:
			{
				CPPCodeGenContext ctx((*priv->o), CPPCodeGenContext::Mode::Implementatinon, (CPPCodeGenContext::Role)priv->role);
				if (!priv->codegen->generateIncludes(0, &ctx, ec))
					return false;
		...
		case Mode::Combo:
			{
				CPPCodeGenContext ctx((*priv->o), CPPCodeGenContext::Mode::AllInOne, (CPPCodeGenContext::Role)priv->role);
				if (!priv->codegen->generateIncludes(0, &ctx, ec))
					return false;
